...
const OurServices = () => {
    ...
    const renderLis = () => {
        ...
        return lis.map((text, i) => <li className={`py-2 px-2 md:px-4 text-base md:text-2xl font-bold mb-4 bg-white rounded shadow-inner text-orange-800 mx-2 transition-all duration-700 hover:shadow hover:scale-105 cursor-pointer`} key={`${i}-${text}`}>{text}</li>)
    }
    ...
    return (
        <div className="w-full mx-auto flex justify-center flex-col lg:my-20 mt-28 md:mt-12 md:px-4">
            ...
            <div className="w-full flex flex-row flex-wrap bg-orange-800 items-center justify-center md:rounded">
                {/* {renderServices()} */}
